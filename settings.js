...
(function(){
  ...
  // 双重初始化保险（防止错过 dbReady 事件）
  var hasSettingsInit = false;
  function trySettingsInit() {
    if (hasSettingsInit) return;
  ...
  if (window._dbReady) {
    ...
    window.addEventListener('dbReady', trySettingsInit);
    setTimeout(trySettingsInit, 500);
  }
    ...
    allKeys.forEach(function(key) {
      if (window.AppDB) {
        if (data.hasOwnProperty(key) && data[key] !== null && data[key] !== undefined && data[key] !== '') {
          AppDB.save(key, data[key], function() { checkDone(); });
        } else {
          AppDB.delete(key, function() { checkDone(); });
        }
      }
    });

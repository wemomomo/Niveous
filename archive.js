...
(function(){
  ...
  function renderArchiveShell() {
    ...
    container.innerHTML = '<div class="archive-page-screen ' + bgClass + '" id="archiveScreenRoot">'
      ...
      + '</div>'
      + '<div class="archive-header-right" id="archiveHeaderActions"></div>'
      + '</div>'

      + '<div class="archive-nav-capsule">'
      + '<div class="nav-glider-pill" id="navGlider" style="transform: translateX(' + (currentTab === 'user' ? '0%' : '100%') + ');"></div>'
      + '<button class="archive-nav-item' + (currentTab === 'user' ? ' active' : '') + '" id="tabUserBtn" type="button"><span>用户档案</span></button>'
      + '<button class="archive-nav-item' + (currentTab === 'char' ? ' active' : '') + '" id="tabCharBtn" type="button"><span>角色档案</span></button>'
      + '</div>'
      + '</div>'
  ...
  function renderSubContent() {
    ...
    var headerActions = document.getElementById('archiveHeaderActions');
    if (!subViewport) return;

    if (currentTab === 'user') {
      var cur = getCurrentUser();
      if (cur) {
        renderStep3(subViewport, headerActions, cur);
      } else if (userList.length > 0) {
        currentUserId = userList[0].id;
        renderStep3(subViewport, headerActions, userList[0]);
      } else {
        renderStep1(subViewport, headerActions);
      }
    } else {
      if (headerActions) {
        headerActions.innerHTML = '<button class="arch-tool-pill" id="actionNewCharBtn" type="button"><svg viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg><span>新建</span></button>'
          + '<button class="arch-tool-pill" id="actionCharListBtn" type="button"><span>角色列表</span></button>';

        document.getElementById('actionNewCharBtn').addEventListener('click', function() {
          if (window.CharacterEngine) CharacterEngine.createNew();
        });
        document.getElementById('actionCharListBtn').addEventListener('click', function() {
  ...
  // 步骤 1：用户空状态凝聚冰晶
  // ==========================================
  function renderStep1(subViewport, headerActions) {
  ...
  // 步骤 3：用户卡片展示与管理
  // ==========================================
  function renderStep3(subViewport, headerActions, cur) {
    if (headerActions) {
      headerActions.innerHTML = '<button class="arch-tool-pill" id="actionNewBtn" type="button"><svg viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg><span>新建</span></button>'
      ...
      drawerCard.querySelectorAll('.drawer-del-btn').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
          ...
          saveCurrentToDB(function() {
            ...
            else renderStep1(document.getElementById('archiveSubViewport'), document.getElementById('archiveHeaderActions'));
